...
machine = 'ieee-le';
[~,~,ext]=fileparts(fileprefix);
if strcmpi(ext,'.nii') 
    fn=fileprefix;
    gzfile=false;
else
    % Check if it is a nii.gz file, if so unzip it
    if length(fileprefix)>7
        if strcmpi(ext,'.gz')
            tmpn=tempname;
            tmpn=[tmpn '.nii'];
            cmd=['gzip -cdf ' fileprefix ' > ' tmpn];
            fprintf('%s\n',cmd);
            system(cmd);
        end
        fn=tmpn;
        ...
        fprintf('ERROR: Input must be a NIFTI .nii or .nii.gz file. You entered %s\n',fileprefix);
        return;
    end
end
